...
export const useUpdateSyarat = (syaratId: string) => {
  ...
  const onSubmitSyarat = async (form: FormSyarat) => {
    ...
    try {
      ...
      toast.success("Syarat berhasil diperbarui");

      return data;
    } catch (error: any) {
      if (error.response.data.message.nama) {
        throw new Error(
          error.response.data.message.nama[0] || "Nama syarat harus diisi",
        );
