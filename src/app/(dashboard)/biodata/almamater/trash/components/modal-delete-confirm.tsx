...
import { useDeleteTrashAlmamater } from "@/handlers/biodata/almamater/trash/delete-trash-almamater";

export const ModalDeleteConfirm = ({ agamaId }: { agamaId: string }) => {
  const [isModalOpen, setIsModalOpen] = useState<boolean>(false);

  const { mutateAsync, isPending } = useDeleteTrashAlmamater();

  const onDeleteuseDeleteTrashAlmamater = async () => {
      ...
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader className="space-y-4">
          ...
          <div className="flex justify-center gap-2">
            ...
            <Button
              onClick={onDeleteuseDeleteTrashAlmamater}
              disabled={isPending}
