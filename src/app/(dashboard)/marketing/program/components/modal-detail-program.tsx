...
import { useUpdateProgram } from "@/handlers/marketing/program/update-program";
import { useGetDetailProgram } from "@/handlers/marketing/program/get-detail-program";

export const ModalDetailProgram: React.FC<{
  isEdit?: boolean;
  programId: number;
}> = ({ isEdit = false, programId }) => {
  const [isModalOpen, setIsModalOpen] = useState<boolean>(false);

  const {
    mutateAsync: onSubmitProgram,
  ...
  const onSubmit = async (data: z.infer<typeof FormSchema>) => {
    await onSubmitProgram(data);
  };

  useEffect(() => {
    if (isModalOpen) {
      getDetailProgram();
      form.setValue("nama", detailProgram?.data?.nama);
    }
  }, [isModalOpen]);
