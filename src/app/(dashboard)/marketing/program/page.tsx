...
import { useGetAllProgram } from "@/handlers/marketing/program/get-all-program";
import { useExportProgram } from "@/handlers/marketing/program/export-program";
import { useImportProgram } from "@/handlers/marketing/program/import-program";
import { ModalAddProgram } from "./components/modal-add-program";

const initialValues: QueryParamsProgram = {
  pageIndex: 0,
  pageSize: null,
  nama: "",
};
...
export default function ProgramPage() {
  const [queryParams, setQueryParams] =
    useState<QueryParamsProgram>(initialValues);
  ...
  useEffect(() => {
    if (isSuccessExportData) {
      FileSaver.saveAs(dataExport, "data-program.xlsx");
  ...
  return (
    <PageWrapper>
      <BasePageTitle title="Daftar Program" />
      <div className="mt-6 flex justify-between">
        ...
        <div className="flex gap-2">
          <Link href="/marketing/program/trash">
            <Button
      ...
      <DataTable
        ...
        total={dataProgram?.data?.total}
        pageCount={dataProgram?.data?.last_page}
