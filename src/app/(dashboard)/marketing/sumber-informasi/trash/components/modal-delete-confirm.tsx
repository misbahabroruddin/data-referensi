...
import { useDeleteTrashSumberInformasi } from "@/handlers/marketing/sumber-informasi/trash/delete-trash-sumber-informasi";

export const ModalDeleteConfirm = ({ programId }: { programId: number }) => {
  const [isModalOpen, setIsModalOpen] = useState<boolean>(false);

  const { mutateAsync, isPending } = useDeleteTrashSumberInformasi();

  const onDeleteSumberInformasi = async () => {
      ...
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader className="space-y-4">
          ...
          <div className="flex justify-center gap-2">
            ...
            <Button
              onClick={onDeleteSumberInformasi}
              disabled={isPending}
