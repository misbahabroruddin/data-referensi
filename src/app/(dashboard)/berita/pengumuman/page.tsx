...
import { useGetAllPengumuman } from "@/handlers/berita/pengumuman/get-all-pengumuman";
import { useExportPengumuman } from "@/handlers/berita/pengumuman/export-pengumuman";
import { useImportPengumuman } from "@/handlers/berita/pengumuman/import-pengumuman";
import { ModalAddPengumuman } from "./components/modal-add-pengumuman";
...
export default function PengumumanPage() {
  ...
  const { data: dataPengumuman, isLoading: isLoadingPengumuman } =
    useGetAllPengumuman(queryParams);
  ...
  useEffect(() => {
    if (isSuccessExportData) {
      FileSaver.saveAs(dataExport, "data-pengumuman.xlsx");
  ...
  return (
    <PageWrapper>
      <BasePageTitle title="Daftar Pengumuman" />
      <div className="mt-6 flex justify-between">
        ...
        <div className="flex gap-2">
          <Link href="/berita/pengumuman/trash">
            <Button
      ...
      <DataTable
        columns={columns}
        data={dataPengumuman?.data}
        total={dataPengumuman?.data?.total}
        pageCount={dataPengumuman?.data?.last_page}
