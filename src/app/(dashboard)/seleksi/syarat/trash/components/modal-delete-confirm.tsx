...
import { useDeleteTrashSyarat } from "@/handlers/seleksi/syarat/trash/delete-trash-syarat";

export const ModalDeleteConfirm = ({ syaratId }: { syaratId: string }) => {
  const [isModalOpen, setIsModalOpen] = useState<boolean>(false);

  const { mutateAsync, isPending } = useDeleteTrashSyarat();

  const onDeleteSyarat = async () => {
    await mutateAsync(syaratId);
    setIsModalOpen(true);
  };
