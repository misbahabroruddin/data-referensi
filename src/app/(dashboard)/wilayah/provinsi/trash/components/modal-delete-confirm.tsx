...
import { useDeleteTrashProvinsi } from "@/handlers/wilayah/provinsi/trash/delete-trash-provinsi";

export const ModalDeleteConfirm = ({ provinsiId }: { provinsiId: string }) => {
  const [isModalOpen, setIsModalOpen] = useState<boolean>(false);

  const { mutateAsync, isPending } = useDeleteTrashProvinsi();

  const onDeleteProvinsi = async () => {
    await mutateAsync(provinsiId);
    setIsModalOpen(true);
  };
