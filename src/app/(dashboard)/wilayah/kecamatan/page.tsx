...
import { useGetAllKecamatan } from "@/handlers/wilayah/kecamatan/get-all-kecamatan";
import { useExportKecamatan } from "@/handlers/wilayah/kecamatan/export-kecamatan";
import { useImportKecamatan } from "@/handlers/wilayah/kecamatan/import-kecamatan";
import { ModalAddKecamatan } from "./components/modal-add-kecamatan";
...
export default function KecamatanPage() {
  ...
  const { data: dataKecamatan, isLoading: isLoadingKecamatan } =
    useGetAllKecamatan(queryParams);
  ...
  useEffect(() => {
    if (isSuccessExportData) {
      FileSaver.saveAs(dataExport, "data-kecamatan.xlsx");
  ...
  return (
    <PageWrapper>
      <BasePageTitle title="Daftar Kecamatan" />
      <div className="mt-6 flex flex-wrap justify-end gap-2 lg:justify-between">
        ...
        <div className="flex items-center justify-end gap-2">
          <Link href="/wilayah/kecamatan/trash">
            <Button
      ...
      <DataTable
        ...
        total={dataKecamatan?.data?.total}
        pageCount={dataKecamatan?.data?.last_page}
