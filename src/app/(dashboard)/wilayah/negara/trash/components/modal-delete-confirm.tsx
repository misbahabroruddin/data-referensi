...
import { useDeleteTrashNegara } from "@/handlers/wilayah/negara/trash/delete-trash-negara";

export const ModalDeleteConfirm = ({ negaraId }: { negaraId: string }) => {
  const [isModalOpen, setIsModalOpen] = useState<boolean>(false);

  const { mutateAsync, isPending } = useDeleteTrashNegara();

  const onDeleteNegara = async () => {
    await mutateAsync(negaraId);
    setIsModalOpen(true);
  };
