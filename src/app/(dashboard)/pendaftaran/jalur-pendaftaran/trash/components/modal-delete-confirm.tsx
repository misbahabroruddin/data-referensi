...
import { useDeleteTrashJalurPendaftaran } from "@/handlers/pendaftaran/jalur-pendaftaran/trash/delete-trash-jalur-pendaftaran";

export const ModalDeleteConfirm = ({ agamaId }: { agamaId: string }) => {
  const [isModalOpen, setIsModalOpen] = useState<boolean>(false);

  const { mutateAsync, isPending } = useDeleteTrashJalurPendaftaran();

  const onDeleteJalurPendaftaran = async () => {
      ...
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader className="space-y-4">
          ...
          <div className="flex justify-center gap-2">
            ...
            <Button
              onClick={onDeleteJalurPendaftaran}
              disabled={isPending}
