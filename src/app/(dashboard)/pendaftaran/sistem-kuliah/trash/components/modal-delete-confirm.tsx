...
import { useDeleteTrashSistemKuliah } from "@/handlers/pendaftaran/sistem-kuliah/trash/delete-trash-sistem-kuliah";

export const ModalDeleteConfirm = ({
  sistemKuliahId,
}: {
  sistemKuliahId: string;
}) => {
  const [isModalOpen, setIsModalOpen] = useState<boolean>(false);

  const { mutateAsync, isPending } = useDeleteTrashSistemKuliah();

  const onDeleteSistemKuliah = async () => {
    await mutateAsync(sistemKuliahId);
    setIsModalOpen(true);
  };
