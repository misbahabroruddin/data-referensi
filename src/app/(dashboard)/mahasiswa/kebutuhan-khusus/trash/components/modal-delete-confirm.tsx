...
import { useDeleteTrashKebutuhanKhusus } from "@/handlers/mahasiswa/kebutuhan-khusus/trash/delete-kebutuhan-khusus";

export const ModalDeleteConfirm = ({ agamaId }: { agamaId: string }) => {
  const [isModalOpen, setIsModalOpen] = useState<boolean>(false);

  const { mutateAsync, isPending } = useDeleteTrashKebutuhanKhusus();

  const onDeleteKebutuhanKhusus = async () => {
      ...
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader className="space-y-4">
          ...
          <div className="flex justify-center gap-2">
            ...
            <Button
              onClick={onDeleteKebutuhanKhusus}
              disabled={isPending}
