...
import { useGetAllRincianBiaya } from "@/handlers/sippm/rincian-biaya/get-all-rincian-biaya";
import { useExportRincianBiaya } from "@/handlers/sippm/rincian-biaya/export-rincian-biaya";
import { useImportRincianBiaya } from "@/handlers/sippm/rincian-biaya/import-rincian-biaya";
import { ModalAddRincianBiaya } from "./components/modal-add-detail-rincian-biaya";
...
export default function RincianBiayaPage() {
  ...
  const { data: dataRincianBiaya, isLoading: isLoadingRincianBiaya } =
    useGetAllRincianBiaya(queryParams);
  ...
  useEffect(() => {
    if (isSuccessExportData) {
      FileSaver.saveAs(dataExport, "data-rincian-biaya.xlsx");
  ...
  return (
    <PageWrapper>
      <BasePageTitle title="Daftar Rincian Biaya" />
      <div className="mt-6 flex justify-between">
        ...
        <div className="flex gap-2">
          <Link href="/sippm/rincian-biaya/trash">
            <Button
          ...
          <ModalAddRincianBiaya />
        </div>
      </div>
      <DataTable
        columns={columns}
        data={dataRincianBiaya?.data}
        total={dataRincianBiaya?.data?.total}
        pageCount={dataRincianBiaya?.data?.last_page}
